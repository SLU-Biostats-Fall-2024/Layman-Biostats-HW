...
observing with the structure command shows me that I have a DF with 60 rows, 1 character variable, and 2 numeric values

Also the Treatment variable looks categorical, so we'll make it a factor variable
...
```{r}
firstcases<-sum(complete.cases(head(df)))
firstcases
```
...
```{r}
sum(is.na(df$Dried_mass_g)) #counts all the 'trues'
```
...
```{r}
sum(!is.na(df$Dried_mass_g)) #counts all the 'falses', ! means 'not'
```
...
```{r}
df_remove_NA_growth <- df%>%
  drop_na(Growth_rate_mm)
```
...
Therefore our statistical method is: One-way ANOVA (because we have \>2 levels of our treatment)
...
```{r}
ggplot(df_All_NA, aes(x=Treatment, y=Growth_rate_mm))+
  geom_boxplot()+
  geom_point()+
  theme_bw()
```
...
I think there is at least one different mean (control diff from L1 + L2) which should return a significant p value.

#### Create a model:

```{r}
sunflower.ModAll <- lm(Growth_rate_mm~Treatment, data=df_All_NA)
sunflower.ModAll
```
...
```{r}
anova(sunflower.ModAll)
```

```{r}
summary(sunflower.ModAll)
```

Practice your interpretation.

Why are there 2 DF for the Treatment effect? Df - 2 bc there are 3 treatments (-1)

Why are there 46 DF for the Residuals? Df - 46 bc there are 49 rows (-3)

How would you write a results statement?

We reject the null hypothesis that the mean growth rates are not statistically different between each treatment group (DF= 2,46 F= 3.6937 p=0.03253)
